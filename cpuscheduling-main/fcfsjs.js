...
function showOutput(input, output, outputDiv) {
    ...
    // Store the input and output data for AI review
    window.schedulingData = { input, output };
    document.getElementById("ai-review-btn").style.display = "inline-block";
}

// Add event listener for AI Review button
...
function analyzeResults(input, output) {
    const aiOutput = document.getElementById("ai-review-output");
    aiOutput.style.display = "block";

    // Get process characteristics
    const processes = input.processId.length;
    const maxBurst = Math.max(...input.totalBurstTime);
    const minBurst = Math.min(...input.totalBurstTime);
    const avgBurst = input.totalBurstTime.reduce((a, b) => a + b, 0) / processes;
    const burstVariance = calculateVariance(input.totalBurstTime);
    const allSameArrival = input.arrivalTime.every(time => time === input.arrivalTime[0]);
    const hasPriorities = input.priority && input.priority.some(p => p !== undefined);

    // Current performance metrics
    const avgWaitingTime = output.averageTimes[2];
    const avgTurnaroundTime = output.averageTimes[1];
    const avgResponseTime = output.averageTimes[3];
    const contextSwitches = output.contextSwitches;
    ...
    if (processes <= 2 && allSameArrival) {
    ...
    } else if (burstVariance > 10 && !allSameArrival) {
        bestAlgorithm = 'SRTF';
        reasons = [
            'High variation in burst times',
        ...
        reasons = [
            'High variation in burst times',
            'All processes available at start',
        ...
        expectedImprovements = [
            ...
            'Fair CPU sharing',
            `Suggested time quantum: ${Math.ceil(avgBurst/2)} units`
        ];
    }
        ...
        <div class="ai-analysis-section">
            ...
            <div class="recommendation-content">
                <div class="current-metrics">
                    ...
                    <ul>
                        <li>Average Waiting Time: ${avgWaitingTime.toFixed(2)}</li>
                        <li>Average Turnaround Time: ${avgTurnaroundTime.toFixed(2)}</li>
                        <li>Average Response Time: ${avgResponseTime.toFixed(2)}</li>
                        <li>Context Switches: ${contextSwitches}</li>
                    </ul>
                </div>
                <div class="recommendation">
                    <h4>Recommended Algorithm: <span class="highlight">${bestAlgorithm}</span></h4>
                    <p><strong>Why this algorithm?</strong></p>
                    <ul>
                    ...
                    ${expectedImprovements.length > 0 ? `
                        ...
                        <ul>
                            ${expectedImprovements.map(imp => `<li>${imp}</li>`).join('')}
                        </ul>
                    ` : ''}
                </div>
            </div>
